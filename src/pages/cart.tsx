...
const CartPage = () => {
  ...
  {productData.length > 0 ? (
    <>
      ...
      <div className="bg-white col-span-1 md:col-span-1 p-4 rounded-lg flex items-center justify-center">
        <CartPayment />
      </div>
    </>
  ) : (
    <div className="bg-white col-span-1 md:col-span-5 flex flex-col items-center justify-center py-5 rounded-lg shadow-lg">
      <h1 className="text-lg md:text-xl font-medium">Your cart is empty!</h1>
      <Link href={"/"}>
        <button className="w-full md:w-52 h-10 bg-amazon_blue text-white rounded-lg text-sm md:text-base font-semibold hover:bg-amazon_yellow hover:text-black">
          Go to Shopping
        </button>
      </Link>
    </div>
